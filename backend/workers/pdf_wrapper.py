...
def wait_for_file_release(file_path: str, timeout: float = 5.0, check_interval: float = 0.1) -> bool:
    """
    等待文件句柄被释放
...
def main():
    ...
    try:
        ...
        try:
            ...
            pdf_doc.close()
            log_json("INFO", f"PDF 页数: {total_pages}")
        except Exception as e:
            log_json("WARNING", f"无法获取 PDF 页数: {e}")
            total_pages = 1
        ...
        # 强制垃圾回收，确保 MinerU 释放所有文件句柄
        import gc
        gc.collect()
        
        # 等待文件系统同步（Windows 可能需要时间释放句柄）
        time.sleep(0.2)
        
        # 查找生成的 Markdown 文件（使用 list 避免迭代器持有句柄）
        ...
        if md_file and os.path.exists(md_file):
            ...
            log_json("INFO", f"准备复制文件: {md_file}")
            
            # Windows 策略：被占用的文件通常可以重命名，利用这一点绕过锁定
            if os.name == 'nt':
                # 先生成一个临时文件名
                temp_name = None
                try:
                    # 在同一目录下重命名（原子操作，通常可以绕过文件锁定）
                    temp_name = md_file + ".tmp_" + str(os.getpid())
                    os.rename(md_file, temp_name)
                    log_json("INFO", f"文件重命名成功: {md_file} -> {temp_name}")
                    
                    # 现在复制重命名后的文件（此时句柄应该指向旧名称）
                    shutil.copyfile(temp_name, str(final_md_path))
                    log_json("INFO", f"已复制 Markdown 到: {final_md_path}")
                    
                    # 删除临时文件
                    try:
                        os.remove(temp_name)
                        log_json("INFO", "临时文件已清理")
                    except:
                        pass
                    ...
                    if temp_name and os.path.exists(temp_name):
                        try:
                            os.rename(temp_name, md_file)  # 尝试恢复原名
                        except:
                            pass
                    
                    log_json("WARNING", "重命名策略失败，回退到等待重试模式...")
